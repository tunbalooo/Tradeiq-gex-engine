...
def test_mobile_chart_boot_is_sequential_and_has_canvas_fallback():
    assert '/static/boot.js?v=17' in INDEX
    assert 'await loadScript("/static/trading_chart.js?v=17")' in BOOT_JS
    assert 'await loadScript("/static/app.js?v=17")' in BOOT_JS
...
def test_v16_service_worker_updates_installed_mobile_apps():
    assert 'tradeiq-v1.7-shell' in SW
    assert '/static/boot.js?v=17' in SW
