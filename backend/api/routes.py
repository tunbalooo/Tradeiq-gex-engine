...
def setup_history(limit: int = Query(100, ge=1, le=500)):
    return storage_service.recent_trades(limit)


@router.get("/scans/history")
def scan_history(limit: int = Query(100, ge=1, le=500)):
    return storage_service.recent_scans(limit)
...
def read_settings():
    ...
    return {
        ...
        "simulated_mode": settings.simulated_mode,
        "active_symbol": profile.symbol,
        "supported_symbols": ", ".join(item["symbol"] for item in instrument_registry.list_public()),
        "dataset": settings.databento_dataset,
        "futures_symbol": profile.futures_continuous,
        "options_parent": profile.options_parent,
        "gex_source": profile.gex_source_label,
        ...
        "direction_switch_confirm_bars": settings.active_direction_switch_confirm_bars,
        "setup_confidence_floor": settings.active_setup_confidence_floor,
        "setup_watch_model_score": settings.active_setup_watch_model_score,
        "entry_model_arm_score": settings.active_entry_model_arm_score,
        ...
        "market_map_enabled": settings.market_map_enabled,
        "market_map_min_display_score": settings.market_map_min_display_score,
        "market_map_min_actionable_score": settings.market_map_min_actionable_score,
        "entry_model_min_score": settings.entry_model_min_score,
        "thesis_lock_max_minutes": settings.active_thesis_lock_max_minutes,
        "move_stop_to_breakeven_after_tp1": settings.move_stop_to_breakeven_after_tp1,
        "partial_exit_percent": settings.partial_exit_percent,
        "multi_market_alerts_enabled": settings.multi_market_alerts_enabled,
        "multi_market_symbols": settings.multi_market_symbols,
        "multi_market_scan_seconds": settings.multi_market_scan_seconds,
        "multi_market_history_refresh_seconds": settings.multi_market_history_refresh_seconds,
        "multi_market_max_data_age_seconds": settings.multi_market_max_data_age_seconds,
        "multi_market_min_model_score": settings.multi_market_min_model_score,
        "databento_live_stale_seconds": settings.databento_live_stale_seconds,
        "databento_live_watchdog_seconds": settings.databento_live_watchdog_seconds,
        "databento_reconnect_initial_seconds": settings.databento_reconnect_initial_seconds,
        "databento_reconnect_max_seconds": settings.databento_reconnect_max_seconds,
        "database": "postgresql/supabase" if settings.database_url.startswith(("postgres", "postgresql")) else "sqlite",
        "admin_protected": not settings.allow_public_admin,
        "claude_analysis_enabled": claude_analysis_service.enabled,
        "claude_model": settings.anthropic_model,
        "finnhub_news_enabled": finnhub_news_service.enabled,
        "finnhub_economic_calendar": finnhub_calendar_service.status(),
    }
