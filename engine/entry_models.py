...
def rank_entry_models(context: ModelContext) -> list[EntryModelScore]:
    ...
    results = [
        ...
        _score(
            ...
            missing=[] if context.gamma_flip is not None else ["gamma flip"], minimum=55, priority=4,
        ),
        _score(
            ...
            missing=[] if context.fvg_mid is not None else ["directional fair value gap"], minimum=52, priority=5,
        ),
        _score(
            ...
            missing=[] if context.zone_mid is not None and displacement else ["displacement-backed order block"], minimum=56, priority=6,
        ),
        _score(
            ...
            missing=[] if trend else ["9/21/55 alignment"], minimum=48, priority=7,
        ),
        _score(
            ...
            missing=[] if context.vwap is not None and vwap else ["confirmed directional VWAP reclaim"], minimum=52, priority=8,
        ),
        _score(
            ...
            missing=[] if displacement and trend else ["directional break with trend alignment"], minimum=54, priority=9,
        ),
        _score(
            ...
            missing=[] if trend else ["aligned trend"], minimum=48, priority=10,
        ),
        _score(
            ...
            missing=[] if s.get("inverse_fvg") and s.get("inverse_fvg_mid") is not None else ["confirmed inverse FVG"], minimum=55, priority=11,
        ),
        _score(
            ...
            missing=[] if s.get("smt_divergence") else ["synchronized comparison-market data"], minimum=58, priority=12,
        ),
    ]
